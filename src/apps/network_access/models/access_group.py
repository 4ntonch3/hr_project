...
class AccessGroupSource(models.Model):
    ...
    arch_stand_guid = models.CharField(
        verbose_name="ID Стенда из архитектурного описания", max_length=128
    )
    arch_component_guid = models.CharField(
        verbose_name="ID Компонента из архитектурного описания", max_length=128
    )
    unactual_at = models.DateTimeField(
        verbose_name="Время потери актуальности", blank=True, null=True
    )
...
class AccessGroup2PrefixSource(models.Model):
    arch_resource_guid = models.CharField(
        verbose_name="ID Ресурса из архитектурного описания", max_length=128
    )
    unactual_at = models.DateTimeField(
        verbose_name="Время потери актуальности", blank=True, null=True
    )
